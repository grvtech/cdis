...
public class Misc {
	...
	public HashMap<String,String> getHeaderGraphCustomReportPeriod(String period){
		...
		switch (period) {
        	case "6":
        		...
        		for(int i=0;i<6;i++) {
        			...
        			result.put(Integer.toString(i), sdf.format(calendar.getTime()));
        			calendar.set(Calendar.DAY_OF_MONTH, 1);
        		}
        		break;
        case "12":
	...
	public  String[] pushArray(String[] array, String[] push) {
	  ...
	  public ArrayList<HashMap<String, Object>> addCriteriaInCriteriaList(ReportCriteria criteria, ArrayList<HashMap<String, Object>> list) {
		  String criteriaName = criteria.getName();
		  String criteriaSection = criteria.getSection();
		  int intSection = Integer.parseInt(criteriaSection);
		  String searchKey = criteriaName;
		  if(criteriaName.indexOf("CollectedDate") >= 0) {searchKey = criteriaName.replaceAll("CollectedDate", "");}
		  ...
		  if(!hasmap) {
			  ...
			  map.put(criteriaName, criteria);
			  if(intSection > 1) {
				  if(criteriaName.indexOf("CollectedDate") >= 0) {
					  map.put(searchKey, null);
					  map.put("flag", "1");
				  }else {
					  map.put(searchKey+"CollectedDate", null);
					  map.put("flag", "0");
				  }
			  }else {
				  map.put("flag", "0");
			  ...
			  for(HashMap<String, Object> item : list) {
				  if(item.containsKey(searchKey)) {
					 item.put(criteriaName, criteria); 
					 if(criteriaName.indexOf("CollectedDate") >= 0) {
						 item.put("flag", "1");
					 }else {
						 item.put("flag", "0");
