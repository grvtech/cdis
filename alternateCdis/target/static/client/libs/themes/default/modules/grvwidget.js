...
import * as printlib from '../../../js/printlib.js';
...
import {configWidget} from './grvwidgetconfig.js';
import {grvpopup} from './grvpopup.js';
import {grvradio} from './grvradio.js';
import {grvdatepicker} from './grvdatepicker.js';
import {grvsurvey} from './grvsurvey.js';
import { shareData } from '../sections/cdis/define.js';

/**
 * some convention : if name contains ,(commas) - the type of the widget is table; 
 * 					 better - resolve condition in  section so the constructor has only single or comma separated and take type fron config
 *                   ---if name contains | (pipe) - is like or operator and it should only be single value name---
 *                   if single value name we take the type of widget ( graph or single) from the config		 
 * 
 */


export class grvwidget{
	static includes = {};
	constructor(name){
		this.name="grvwidget";
		this.loadstyle();
		...
		if(name.indexOf(",") >= 0 ){
			this.type = "table";
			let parts = this.valueName.split(",");
			$.each(parts, function(i,part){
				configs.push(configWidget[part]);
			});
		}else{
			configs.push(configWidget[this.valueName]);
			this.type = configs[0].type; 			
		}
		this.configs = configs;
		this.container = $("div [data='"+name+"']");
		this.dates = [];
		this.data = [];
		this.images = [];
		if(this.type == 'graph'){this.createGraphWidget();}
		else if(this.type == 'table'){this.createTableWidget();}
		else if(this.type == 'single'){this.createSingleWidget();}
		this.patient = patientlib.getPatientObjectData("record");
		this.diabet = patientlib.getPatientObjectData("diabet");
	}
		...
		$.each(moduleconfig.includes, function(i,mod){
			...
			if(mod.module == self.name){
				$.each(mod.libs, async function(j,lib){
					try {
					  grvwidget.includes[lib.alias] = await import(lib.file);
					} catch (error) {
	...
	createGraphWidget(){
		...
		const self = this;
		try{
			if(this.valueName.indexOf(",") >= 0 ){
				let parts = this.valueName.split(",");
				$.each(parts,function(i,part){
					let va = patientlib.getValueSectionArray(self.configs[i].section,part);
					self.data.push(va);
					$.each(va,function(x,v){
						if(!self.dates.includes(v.date))self.dates.push(v.date);
					});
				}) 
			}else{
				let va = patientlib.getValueSectionArray(self.configs[0].section,this.valueName);
				
				if(va.length == 1){
					if(va[0].idvalue == 0){ }
					else{
						this.data.push(va);
						self.dates.push(va[0].date);
					}
				}else if(va.length > 1){
					this.data.push(va);
					$.each(va,function(x,v){
		...
		//this.data = patientlib.getValueSectionArray(this.configs[0].section,this.valueName);	
		}catch(error){
			console.log("Errrroooorrrr : "+ error.message);
		}
		this.dates.sort(genericlib.compareDateDesc);
		this.container.empty();
		var gc = $("<div>",{class:"grvwidget-graph-container"}).appendTo(this.container);
		var gch = $("<div>",{class:"header"}).appendTo(gc); // split in 2 title and menu
		let u = (this.configs[0].unit!='')?"("+this.configs[0].unit+")":"";
		$("<div>",{class:"title"}).appendTo(gch).html(this.configs[0].label+" <span class='unit'>"+u+'</span>');
		var gchm = $("<div>",{class:"menu"}).appendTo(gch);
		
		$("<div>",{id:this.id+"-table-button",class:"cdisCisButton",'data-toggle':"tooltip",title:"Open data table"})
			.appendTo(gchm)
			.append($("<i>",{class:"fa-solid fa-table"}))
			.on("click",{self: this, button:$("#"+this.id+"-table-button")},this.toggleGraphTable);
		$("<div>",{id:this.id+"-form-button",class:"cdisCisButton",'data-toggle':"tooltip",title:"Add new data"})
			.appendTo(gchm)
			.append($("<i>",{class:"fa-solid fa-plus"}))
			.on("click",{self: this, action:"add"},this.toggleGraphForm);
		$("<div>",{id:this.id+"-history-button",class:"cdisCisButton",'data-toggle':"tooltip",title:"Open history graph"})
			.appendTo(gchm)
			.append($("<i>",{class:"fa-solid fa-chart-line"})) 
			.on("click",{self:this},this.plotGraphHistory);
		$("<div>",{id:this.id+"-print-button",class:"cdisCisButton",'data-toggle':"tooltip",title:"Print graph"})
			.appendTo(gchm)
			.append($("<i>",{class:"fa-solid fa-print",ariaHidden:"true"}))
			.on("click",{self:this},this.printGraph);
		
		var gcb = $("<div>",{class:"body"}).appendTo(gc);
		
		$("<div>",{class:"graph",id:this.id+"-graph"}).appendTo(gcb);
		if((this.data.length == 1 && this.data[0].idvalue == 0) || this.valueName == "" || this.data.length == 0){
			$("<div>",{class:"graph"}).text("No data available").appendTo($("#"+this.id+"-graph"));
		}else{
			this.buildGraphGraph();	
		}
		
		
		$("<div>",{class:"table",id:this.id+"-table"}).appendTo(gcb);
		this.buildGraphTable();
		
		$("<div>",{class:"form",id:this.id+"-form"}).appendTo(gcb);
		//this.buildGraphForm();
	}
	
	
	toggleGraphTable(event){
		let button = $("#"+event.data.self.id+"-table-button");
		
		let table = $("#"+event.data.self.id+"-table");
	...
	buildGraphTable(){
		const self = this;
		var container = $('#'+this.id+'-table');
		container.empty();
		
		
		let names = [];
		if(this.valueName.indexOf(",") >= 0 ){
			names = this.valueName.split(",");	
		}else{
			names.push(this.valueName);
		}
		

		const table = $("<table>",{cellpadding:0,cellspacing:0,border:0}).appendTo(container);
		const headerTable = $("<tr>").appendTo($("<thead>").appendTo(table));
		$.each(names,function(i,dataArr){
			$("<th>").appendTo(headerTable).text(self.configs[i].label);
		})
		$("<th>").appendTo(headerTable).text("Date");
		$("<th>").css("width","25px").appendTo(headerTable).html("&times;").css("cursor","pointer").on("click",{self: this},self.toggleGraphTable);
		
		let body = $("<tbody>").appendTo(table);
		console.log(self.dates)
		console.log(self.data)
		$.each(self.dates,function(index,valueObject){
			let line = $("<tr>",{class:"line"}).appendTo(body);
			let ids = "";
			$.each(self.data, function(y,vArray){
				console.log("vArray")
				console.log(vArray)
				//normaly I should only have ne date per value so to be safe create the cell now and assign last value to it
				let cell =  $("<td>",{class:"data"}).appendTo(line);
				let val = "";
				let idv = names[y]+"_0";
				$.each(vArray, function(z,vValue){
					if(vValue.date == valueObject){
						val = vValue.value;
						idv = names[y]+"_"+vValue.idvalue;
					} 
				});
				cell.text(val).attr("idvalue",idv);
				ids+=idv+",";
			});
			$("<td>",{class:"data",idvalue:"date_0"}).text(valueObject).appendTo(line);
			$("<td>",{ids:ids,class:"delete"})
				.appendTo(line)
				.append($("<i>",{class:"fa-regular fa-trash-can"}))
				.on("click",function(event){
					event.stopPropagation();
					if(appDefine.isDemo){alert("This function si not available in demo mode");}
					else{
						if(self.valueName.indexOf(",") >=0){
							let txt = "<p>Which value from the specific date would you like to delete?</p><div id='radio-data' type='grvradio'></div>";
							let rconfig =  {container:"radio-data",elements:[]};
							let elems = [];
							rconfig.elements.push({label:"All values of this date",value:names.join(","),default:0});
							
							$.each(names, function(a,b){
								let elem = {label:self.configs[a].label,value:b,default:0};
								rconfig.elements.push(elem);
							})
							var idsValue = $(this).attr("ids");
							let config = {width:400,height:220,
								container:"grvWraper",
								buttons:[{"text":"Close","action":"closeGRVPopup","alias":"this"},
										 {"text":"Delete","action":"deleteTableValues","alias":"clib","params":[idsValue]}],
								content:txt,
							...
							setTimeout(function(){
								...
								shareData.radios.push({name:"deleteTableValues",object:r});
							},300);	
						}else{
							var idsValue = $(this).attr("ids");
							let config = {width:300,height:200,
								container:"grvWraper",
								buttons:[{"text":"Close","action":"closeGRVPopup","alias":"this"},
										 {"text":"Delete","action":"deleteGraphValues","alias":"clib","params":[idsValue]}],
								content:"This value will be deleted! Are you sure?",
								title:"Delete value from table"
							}
							new grvpopup(config);
						}
			...
			line.on("click", function(event){
				if(appDefine.isDemo){
					...
					console.log("clik on the line");
					let predata = [];
					let alldata = $(this).children(".data");
					$.each(alldata,function(xx,vv){
						let ps = $(vv).attr("idvalue").split("_");
						let iv = ps[0];
						let v = $(vv).text();
						predata.push({question:iv,answers:[v],idvalue:ps[1]});
					})
					let e = {data:{self:self, action:"edit",predata:predata}} 
					self.toggleGraphForm(e);
				}    
			});
		});
	}
	...
	toggleGraphForm(event){
		
		let button = $("#"+event.data.self.id+"-form-button");;
		let table = $("#"+event.data.self.id+"-form");
		...
		if(button.hasClass("selected")){
			...
			if(event.data.action == "add"){
				const config = {
					container:event.data.self.id+"-form",
					surveyid:event.data.self.valueName,
					...
					completeLabel:"Save Value",
					closeSurveyCallback:function(id){event.data.self.closeGraphForm(id,event.data.self);},
					completeSurveyCallback:function(result){event.data.self.saveGraphForm(result,event.data.self);}
				};
				
				const s = new grvsurvey(config);
				shareData.surveys.push({name:self.nameValue,object:s});
				setTimeout(()=>{$("#"+s.id+" .grvsurvey-header .title").text("Add data");},100);
			}
			if(event.data.action == "edit"){
				...
				const s = new grvsurvey(config,event.data.predata);
				
				shareData.surveys.push({name:self.nameValue,object:s});
				setTimeout(()=>{$("#"+s.id+" .grvsurvey-header .title").text("Edit data");},100);
			}
			
		} 
	}
	
	closeGraphForm(id, object){
		$("#"+id).css("display","none");
		let button = $("#"+object.id+"-form-button");
		button.removeClass("selected");
		let index = null;
		$.each(shareData.surveys,(i,o)=>{if(o.name == self.nameValue)index = i;})
		shareData.surveys.splice(index,1);
	}
		
	saveGraphForm(result,object){
		console.log(result);
		if(object.valueName.indexOf(",") >= 0){
			let parts = object.valueName.split(",");
			$.each(parts,(i,part)=>{
				let vDate="";
				let vValue = "";
				let id = "0";
				$.each(result.result,(j,r)=>{
					if(r.question == "date")vDate = atob(r.answers[0].text);
					if(r.question == part){
						if(r.type == "checkbox"){
							$.each(r.answers, (k,a)=>{
								vValue += atob(a.text)+",";
							...
							//take first from answers
							vValue = atob(r.answers[0].text);
							id = r.answers[0].idvalue;	
						}
					}
				})
				//alert(id+" "+part+" "+vDate+" "+vValue)
				patientlib.saveValue(id,part,vDate,vValue);
				this.redraw();
			}) 
		}else{
			let vDate="";
			...
			$.each(result.result,(j,r)=>{
				if(r.question == "date")vDate = atob(r.answers[0].text);
				if(r.question == object.valueName){
					if(r.type == "checkbox"){
						$.each(r.answers, (k,a)=>{
							vValue += atob(a.text)+",";
							id = a.idvalue;
			...
			//alert(id+" "+object.valueName+" "+vDate+" "+vValue);
			patientlib.saveValue(id,object.valueName,vDate,vValue);
			this.redraw();
		}
	}
	...
	buildGraphGraph(){
		const self = this;
		const g = graphlib.drawGraphWidget(this);
		setTimeout(function() {
		        var base64Image = g.toBase64Image();
		        // Use base64Image here
				let imgObject = {name:self.id,image:base64Image};
				self.images.push(imgObject);
	...
	createTableWidget(){
		...
		let title = this.container.attr("title");
		var gc = $("<div>",{class:"grvwidget-table-container"}).appendTo(this.container).css("height",this.container.css("height"));
		let header = $("<div>",{class:"header"}).appendTo(gc).append($("<div>",{class:"title"}).text(title));
		let menu = $("<div>",{class:"menu"}).appendTo(header);
			
		$("<div>",{class:"cdisCisButton",'data-toggle':"tooltip",title:"Add new data",id:self.id+"-form-button"})
			.append($("<i>",{class:"fa-solid fa-plus"}))
			.on("click",{self:this, button:$("#"+self.id+"-form-button"), action:"add"}, self.toggleTableForm)
			.appendTo(menu);
		$("<div>",{class:"cdisCisButton",'data-toggle':"tooltip",title:"Open history graph",id:this.id+"-history-button"})
			.append($("<i>",{class:"fa-solid fa-chart-line"}))	
			.on("click",{self:this},this.plotGraphHistory)
			.appendTo(menu);
		$("<div>",{class:"cdisCisButton",'data-toggle':"tooltip",title:"Print graph",id:this.id+"-print-button"})
			.append($("<i>",{class:"fa-solid fa-print"}))
			.on("click",{self:this},this.printGraph)
			.appendTo(menu);
	
		let body = $("<div>",{class:"body"}).appendTo(gc);
		$("<div>",{class:"table",id:this.id+"-table"}).appendTo(body);
		this.buildTableTable();
				
		$("<div>",{class:"form",id:this.id+"-form"}).appendTo(body);
		
	}

	toggleTableForm(event){
		let button = $("#"+event.data.self.id+"-form-button");
		let table = $("#"+event.data.self.id+"-form");
		...
		if(button.hasClass("selected")){
			...
			if(event.data.action == "add"){
				const config = {
					...
					date:genericlib.formatDate(new Date(), "yyyy-mm-dd"),
					completeLabel:"Save Value",
					closeSurveyCallback:function(id){event.data.self.closeTableForm(id,event.data.self);},
					completeSurveyCallback:function(result){event.data.self.saveTableForm(result,event.data.self);}
				};
				const s = new grvsurvey(config);
				shareData.surveys.push({name:self.nameValue,object:s});
				setTimeout(()=>{$("#"+s.id+" .grvsurvey-header .title").text("Add data");},100);
	...
	buildTableTable(){
		const self = this;
		let container = $("#"+this.id+"-table");
		//we will use table so we can keep widths
		let names = [];
		if(this.valueName.indexOf(",") >= 0 ){
			names = this.valueName.split(",");	
		}else{
		...
		let heightWidgetContainer = container.parent().parent().height();
		let heightBody = heightWidgetContainer - 35;
		container.css("height",heightBody+"px");
		const table = $("<table>",{cellpadding:0,cellspacing:0,border:0}).appendTo(container);
		const headerTable = $("<tr>").appendTo($("<thead>").appendTo(table));
		$.each(names,function(i,dataArr){
			$("<th>").appendTo(headerTable).text(self.configs[i].label);
		...
		$("<th>").appendTo(headerTable).text("Date");
		$("<th>").css("width","25px").appendTo(headerTable).text("");
		let body = $("<tbody>").appendTo(table);
		$.each(self.dates, function(x,date){
			let line = $("<tr>",{class:"line"}).appendTo(body);
			let ids = "";
			$.each(self.data, function(y,vArray){
				//normaly I should only have ne date per value so to be safe create the cell now and assign last value to it
				...
				let c = val;
				if(self.configs[y].values_object){
					val = self.configs[y].values_object[val]
				}
				if(self.configs[y].limits){
					let limits = self.configs[y].limits;
					$.each(limits.stages, (x,stage)=>{
						if(c <= stage.max && c >= stage.min){
							val = "<span style='background:"+stage.color+";padding:3px;'>"+val+"</span>";
						}
					})
				}
				cell.html(val).attr("idvalue",idv);
				ids+=idv+",";
			});
			$("<td>",{class:"data",idvalue:"date_0"}).text(date).appendTo(line);
			...
			line.on("click", function(){
				if(appDefine.isDemo){
					...
					let e = {data:{self:self, button:$("#"+self.id+"-form-button"), action:"edit",predata:predata}} 
					self.toggleTableForm(e);
				}
	...
	plotGraphHistory(event){
		const self = event.data.self;
		
		let txt = '<div id="'+self.id+'-graph-history" class="grvwidget-history-container"></div>';
		let config = {width:900,height:600,
						container:"grvWraper",
						buttons:[{"text":"Close","action":"closeGRVPopup","alias":"this"}],
						content:txt,
		...
		new grvpopup(config);
		const g = graphlib.drawGraphWidget(self,1);
		let imgObject = {name:self.id+"-history",image:g.toBase64Image()};
		self.images.push(imgObject);
	}
		
	printGraph(event){
			const self = event.data.self;
			$("<div>",{id:self.id+"-graph-history"}).appendTo($("body"))
			...
			setTimeout(function(){
				let imgObject = {name:self.id+"-history",image:g.toBase64Image()};
				self.images.push(imgObject);
				$("#"+self.id+"-graph-history").remove();
				printlib.printWidget(self);
			},500)
