...
public String executeReport(final HttpServletRequest request){
    ...
    /*instead of array list of criteria I should put arraylist of arrays or hashmap of criterias*/
    //ArrayList<ReportCriteria> lcs = new ArrayList<ReportCriteria>();
    ArrayList<HashMap<String,Object>> lcs = new ArrayList<HashMap<String,Object>>();
    //ArrayList<ReportCriteria> slcs = new ArrayList<ReportCriteria>();//subcriterias
    ArrayList<HashMap<String,Object>> slcs = new ArrayList<HashMap<String,Object>>();
    ...
    if(type.equals("list")){
    	...
    	Hashtable<HashMap<String,Object>, ArrayList<HashMap<String,String>>> report = new Hashtable<>();
	    for(int i=0;i<lcs.size();i++){ 
	    	...
	    	for(int ii=0;ii<criteriaSet.size();ii++){
	    		HashMap<String,String> line = criteriaSet.get(ii);
	    		String idp = line.get("idpatient");
	    ...
	    for(int x=0;x<idpatients.size();x++){
	    	...
	    	Hashtable<ReportCriteria, ArrayList<HashMap<String,String>>> patientMap = new Hashtable<>();
	    	
	    	for(int y=0;y<lcs.size();y++){
	    		HashMap<String,Object> item = lcs.get(y);
	    		Set<String> s = item.keySet();
	    		ReportCriteria rccDate = null;
	    		ReportCriteria rcc = null;
	    				
	    		for(String k : s) {
	    			if(k.indexOf("CollectedDate") >= 0 ) {
	    				rccDate = (ReportCriteria)item.get(k);
	    			}
	    			if(k.indexOf("CollectedDate") < 0 && k.indexOf("flag") < 0) {
	    				rcc = (ReportCriteria)item.get(k);
	    			}
	    		}
	    		
	    		String rccName = rcc.getName();

	    		ArrayList<HashMap<String,String>>  rcset =  report.get(item);
	    		ArrayList<HashMap<String,String>>  rcsetPatient =  new ArrayList<>();
	    		for(int z=0;z<rcset.size();z++){
	    			HashMap<String,String> rcLine = rcset.get(z);
	    			String rcIdPat = rcLine.get("idpatient");
	    	...
	    	HashMap<String,String> setLine = new HashMap<>();
	    	for(int qq=0;qq<lcs.size();qq++){
	    		HashMap<String,Object> item = lcs.get(qq);
	    		Set<String> s = item.keySet();
	    		
	    		ReportCriteria r = null;
	    		ReportCriteria rDate = null;
	    		String flag = "0";
	    		for(String k : s) {
	    			if(k.indexOf("CollectedDate") >= 0)rDate = (ReportCriteria)item.get(k);
	    			if(k.indexOf("CollectedDate") < 0 && k.indexOf("flag") < 0)r = (ReportCriteria)item.get(k);
	    		...
	    		if(rpset.size() > 0){
	    			for(HashMap<String,String> line : rpset) {
	    				...
	    				for(String key : keys) {
	    					Set<String> linekeys = line.keySet();
	    					//if(!linekeys.contains(key)) {
	    						if(r.getSection().equals("1")) {
	    							setLine.put(key, line.get(key));
	    						}else {
	    							if(key.equals("datevalue")) {
	    								setLine.put(r.getName()+"CollectedDate", line.get(key));
	    							} else if(key.equals("value")) {
	    								setLine.put(r.getName(), line.get(key));
	    							}else {
	    								setLine.put(key, line.get(key));
	    							}
	    						}
    	...
    	HashMap<String, Object>  criteriaMap = lcs.get(0);
    	Set<String> keys = criteriaMap.keySet();
    	for(String key : keys) {
    		if(key.indexOf("CollectedDate") < 0 && !key.equals("flag")) {
    			criteria = (ReportCriteria)criteriaMap.get(key);
    		}
    	}
    	...
    	if(criteria.getName() != null) {
    		...
    		ArrayList<HashMap<String,ArrayList<String>>> idpatientsWithCriteria = cdisdb.getIdPatientsForCustomReportGraph(criteria,filter,header);
    		
    		for(int i=0;i<slcs.size();i++){ 
        		HashMap<String, Object>  subcriteriaMap = slcs.get(i);
     	    	ReportCriteria subcriteria = null;
     	    	Set<String> kkeys = subcriteriaMap.keySet();
     	    	for(String kkey : kkeys) {
     	    		if(kkey.indexOf("CollectedDate") < 0 && !kkey.equals("flag")) {
     	    			subcriteria = (ReportCriteria)subcriteriaMap.get(kkey);
     	    		}
     	    	}
     	    	idpatientsWithCriteria = cdisdb.executeReportCriteriaGraph(subcriteriaMap,filter,header,idpatientsWithCriteria,criteria);
     	    }
    		
			HashMap<String,String> line1 = new HashMap<>();
			HashMap<String,String> line2 = new HashMap<>();
    		for(int x=0;x<idpatientsWithCriteria.size();x++) {
    			HashMap<String,ArrayList<String>> m = idpatientsWithCriteria.get(x);
    			Set<String> kks = m.keySet();
    			if(kks.size() == 1) {
    				Map.Entry<String,ArrayList<String>> entry = m.entrySet().iterator().next();
    				...
    				for(String kk : kks) {
    					String[] kkparts = kk.split("_"); 
    					if(kkparts[1].equals(p1)) {
    						int n1 = m.get(kk).size();
    						line1.put(kk, Integer.toString(n1));
    					}else if(kkparts[1].equals(p2)) {
    						int n2 = m.get(kk).size();
    						line2.put(kk, Integer.toString(n2));
    					}
    				}
    			}
    		}
    ...
    if(totals.size() > 0) {
    	reportObject.put("totals", totals);
    	JsonElement jtotals = gson.toJsonTree(totals);
    	jObject.add("totals", jtotals);
    }
	...
	if(historyFolder.exists()) {
		...
		for(int i=0;i<files.length;i++) {
			try {
				...
				rep.put("period", jo.get("period").getAsString());
				rep.put("graphtype", jo.get("graphtype").getAsString());
				rep.put("criteria", jo.get("criteria").getAsJsonArray().toString());
				rep.put("subcriteria", jo.get("subcriteria").getAsJsonArray().toString());
				userReports.add(rep);
				jr.close();
			} catch (IOException e) {
				e.printStackTrace();
